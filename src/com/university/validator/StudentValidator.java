...
public class StudentValidator {
   ...
   */
  public int validateStudentId(int studentId, Student updatedStudent) throws StudentIdFormatException, InvalidStudentIdException{
    if (String.valueOf(studentId).length() != 8){
      throw new StudentIdFormatException();
    }
   ...
   *
   * 1993년부터 법적으로 성을 제외한 이름은 5글자 이내로 제한했습니다.
   * 하지만 성이 한 글자가 아닌 두 글자 성이 있을 경우를 위해 7글자 제한했습니다.
   *
  ...
  /**
   * 등록하는 학생 전화번호는 "010-XXXX-XXXX" 패턴으로 등록되도록 합니다.
   * 또한, 전화번호가 이미 등록된 학생들과 겹치지 않도록 확인합니다.
   *
  ...
  /**
   * 학생 정보를 업데이트할 때 전화번호는 등록된 다른 학생들의 전화번호와 중복되는지 확인합니다.
   * 전체 학생들을 하나씩 iterate하면서 업데이트 될 전화번호가 중복되는지 확인하고 아닐 경우 예외를 던집니다.
   *
   * 만약 사용자가 학생 전화번호를 업데이트하지 않고 그대로 쓸 경우
   * 중복되는 전화번호가 업데이트 되는 학생 객체인지 확인합니다.
   *
  ...
  /**
   * Helper Method
   * 입력된 전화번호가 "010-XXXX-XXXX" 패턴에 맞는지 확인합니다.
   *
   * @param phoneNumber 사용자가 입력하는 전화번호.
  ...
  /**
   * Helper Method
   * 입력된 전화번호가 다른 학생들의 전화번호와 중복되는지 확인합니다.
   *
   * @param phoneNumber 사용자가 입력하는 전화번호.
   * @param students 프로그램에 등록된 전체 학생들의 객체들.
